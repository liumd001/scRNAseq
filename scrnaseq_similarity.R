...
###
###
###   Analysis of prostate cancer scRNAseq data
###
###         
######################################################################
...
geneann  = geneann %>%
  mutate(genecat = ifelse(Symbol %in% tgenes, "prostate", genecat))
...
write.csv(dta_similarity,file.path(out_dir,"JaccardMatrix_byDS.csv"), row.names = FALSE)
...
dta3 = dta_similarity %>%
  ...
  distinct() %>%
  mutate(genecat2 = recode(genecat2, "hk" = "red","prostate" = "green","random" = "blue")) 
...
file.copy(temp_file, file.path(out_dir, "Scatterplot_Jaccard_overview.png"), overwrite = TRUE)
...
file.copy(temp_file, file.path(out_dir, "Scatterplot_OverallConcordance_overview.png"), overwrite = TRUE)
...
file.copy(temp_file, file.path(out_dir, "Scatterplot_PositiveConcordance_overview.png"), overwrite = TRUE)
...
if (FALSE){
  ...
  write.csv(dta_similarity_bySubject, file.path(out_dir, "JaccardSimilarity_BySubject.csv"), row.names = FALSE)
} #ENDOFIFELSE

dta_similarity_bySubject = read.csv(file.path(out_dir, "JaccardSimilarity_BySubject.csv"))
...
dta3 = dta_similarity_bySubject %>%
  ...
  left_join(geneann %>%
              ...
              rename(genecat2 = genecat), by = c("g1" = "Symbol")) %>%
  distinct() %>%
  mutate(genecat3 = recode(genecat, "hk" = "red","prostate" = "green","random" = "blue")) %>%
